  ...
  return JSON.stringify(
    {
      ...
      commitHeadlines: input.commitHeadlines ?? [],
...
export async function explainBuildSignalWithGemini(
  ...
  const prompt = [
    ...
    "No buy/sell advice.",
    "Style: easy-to-read update, not a compliance memo.",
    "Each field MUST be one short sentence (max ~22 words).",
    "whatHappened MUST quote the actual commitHeadlines when present. Do not write only 'product feature work'.",
    "tokenRelation = how this does/doesn't relate to the tracked token. Do not repeat market cap or liquidity.",
    "whyItMayMatter = use shortDescription or trackingReason. Do not write 'still showing public shipping activity'.",
    "whatWeDoNotKnow = one concrete unknown from THESE facts. Never write 'doesn't prove usage, revenue, or a contract change'.",
    "whatToWatchNext = one concrete next public sign. Never write 'release/product note, or another clear public commit'.",
    "If projectStatus is migration, say the tracked contract may not be the live token.",
